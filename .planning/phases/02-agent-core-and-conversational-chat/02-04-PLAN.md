...
Note: If migration 002 (memory_conversations) also exists and was applied first,
change Revises to "002" to maintain a linear history.
"""
...
revision = "003"
down_revision = "001"  # Update to "002" if 002 was applied first
branch_labels = None
depends_on = None
...
If both 002 (memory_conversations) and 003 (user_credentials) both have `down_revision = "001"`, Alembic will report a merge conflict. Fix by checking current head:

```bash
.venv/bin/alembic heads
```

If there are two heads (one for 002, one for 003), create a merge migration:
```bash
.venv/bin/alembic merge -m "merge 002 and 003" 002 003
```

Then run: `.venv/bin/alembic upgrade head`
