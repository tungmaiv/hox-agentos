...
**Current focus:** v1.2 Developer Experience — defining requirements

## Current Position

Milestone: v1.2 Developer Experience — STARTED 2026-03-02
...
- [08-01]: Alloy uses loki.source.docker (not file tailing) — structlog JSON written to stdout; Docker captures it via /var/run/docker.sock
- [08-01]: cAdvisor has no host port binding — Prometheus scrapes cadvisor:8080 on blitz-net only; Keycloak also on 8080 but different DNS name, no collision
- [08-01]: Grafana GF_AUTH_DISABLE_LOGIN_FORM=false — local admin fallback preserved to prevent lockout when Keycloak is down
...
- [Phase 08-03]: Alert threshold ### Decisions
