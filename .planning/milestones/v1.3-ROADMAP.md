...
- ✅ **v1.3 Production Readiness & Skill Platform** — Phases 15–25 (shipped 2026-03-14)
...
- [x] **Phase 24: Unified Registry, MCP Enhancement & Skill Import Adapters** — Unified registry_entries table, MCP catalog, admin UI 4-tab restructure, LLM admin config, security scan service (completed 2026-03-13)
- [x] **Phase 25: Skill Builder Tool Resolver** — resolve_tools LangGraph node, pending_activation status, tool gap auto-resolution, bell icon notification (completed 2026-03-14)
...
### Phase 24: Unified Registry, MCP Enhancement & Skill Import Adapters
**Goal**: All agent/skill/tool/MCP server definitions are stored in a single unified registry table; admin UI is restructured into a 4-tab layout; MCP servers have a pre-built catalog; LLM model aliases are configurable at runtime.
**Depends on**: Phase 23 (builder artifacts must write to registry)
**Key deliverables**:
- `registry_entries` table (migration 029 `c12d84fc28f9`) — unified storage replacing `agent_definitions`, `skill_definitions`, `tool_definitions`, `mcp_servers`
- `mcp_server_catalog` table (migration 030 `617b296e937a`) — pre-built MCP server catalog
- `/api/registry/*` unified CRUD endpoints
- `/api/admin/llm/models` — runtime LLM model alias management
- `/api/admin/system/health` + `/rescan-skills` — system health and retroactive security scan
- Admin UI restructured to 4-tab layout: Registry | Access | System | Build
- Auth tokens stored as hex-encoded AES-256-GCM blobs in `config['auth_token_hex']`

Plans:
- [x] 24-05: Admin system routes (health, rescan-skills) + SecurityScanner integration
- [x] 24-06: Admin UI 4-tab restructure + LLM admin config routes
- [x] 24-07: Unified registry migration + openapi_bridge auth token hex encoding

### Phase 25: Skill Builder Tool Resolver
**Goal**: The artifact builder resolves required tools before generating skill content; skills with missing tools are saved as drafts with explicit gap tracking; when a missing tool is created, the skill is auto-promoted to pending_activation.
**Depends on**: Phase 24 (unified registry for skill/tool storage)
**Key deliverables**:
- `resolve_tools` LangGraph node in artifact_builder (procedural skills only)
- `tool_gaps` field in `RegistryEntry.config` JSONB
- `pending_activation` status: auto-promoted when missing tool is created
- Bell icon in admin nav showing `pending_activation` skills count
- `create_skill` defaults to `status=draft`
- New endpoints: `GET /api/admin/tools/check-name`, `PATCH /api/admin/tools/{id}/activate-stub`, `PATCH /api/admin/skills/{id}/activate` (422 if tool_gaps present)

Plans:
- [x] 25-01: resolve_tools node + tool_gaps tracking + required_permissions injection
- [x] 25-02: Activation gate (422 on tool_gaps) + gap summary formatting + ToolHandler auto-promote
- [x] 25-03: Bell icon in admin nav for pending_activation count
- [x] 25-04: builder-save migrated to UnifiedRegistryService (UAT gap fix)
- [x] 25-05: Bell dropdown + formState skill_type fix in builder
- [x] 25-06: create_skill default status=draft enforcement
...
| 24. Unified Registry & MCP Enhancement | v1.3 | 3/3 | ✅ Complete | 2026-03-13 |
| 25. Skill Builder Tool Resolver | v1.3 | 6/6 | ✅ Complete | 2026-03-14 |
