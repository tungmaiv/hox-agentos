/**
 * Catch-all proxy for /api/storage/* backend routes.
 *
...
async function proxyRequest(
  ...
  const backendPath = `/api/storage/${path.join("/")}`;
  ...
  if (["POST", "PUT", "PATCH", "DELETE"].includes(request.method)) {
    if (contentType?.includes("multipart/form-data")) {
      ...
      // Keep Content-Type (already set above with boundary) so backend can parse the form.
